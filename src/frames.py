...
class Frames:
    ...
    def __init__(self, mainObj, MainWin, wWidth, wHeight, function, Object, xAxis=10, yAxis=10):
        ...
        self.MainWindow.title("Brain Tumor Classification")
        ...
        self.btnClose.place(x=int(0.8*wWidth), y=int(0.85*wHeight))
        # self.btnView = tkinter.Button(self.winFrame, text="View", width=8, command=lambda: self.NextWindow(self.method))
        # self.btnView.place(x=int(0.6*wWidth), y=int(0.7*wHeight))
    ...
    def displayImage(self):
        imgTk = self.image.resize((400, 400), Image.Resampling.LANCZOS)
        imgTk = ImageTk.PhotoImage(image=imgTk)
        self.image = imgTk
        self.labelImg = tkinter.Label(self.winFrame, image=self.image)
        self.labelImg.place(x=0.1*self.wWidth, y=0.1*self.wHeight)